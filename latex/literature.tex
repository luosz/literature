...
Correa and Ma \cite{correa_visibility-driven_2009} introduced visibility histograms to guide transfer function design for both manual and automatic adjustment.
%Visibility histograms, which summarize the distribution of visibility of voxels from a given viewpoint, are a powerful feedback mechanism for volume visualization \cite{emsenhuber_visibility_2008}.
...
%They described a scheme that allows users to specify a desired visibility for features of interest and subsequently the opacity transfer function is optimized using an active set algorithm \cite{polyak_conjugate_1969}.
...
%Peachey et al. \cite{peachey_parallel_2009} presented an approach to parallelize quasi-Newton methods over a cluster of computers in order to accelerate convergence.
...
%%Opacity-driven volume clipping for slice of interest (SOI) visualization of multi-modality PET-CT volumes
